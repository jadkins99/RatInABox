"""Post-FTA ReLU heatmaps collapsed to 20 features (sum over 11 tiles per feature)."""
import sys, os
...
def compute_relu_per_feature(relu_acts, total_tiles, n_features):
    """Collapse 220 ReLU outputs to 20 by summing over tiles per feature.

    ReLU output layout: [batch, total_tiles * n_features]
    Reshaped: [batch, total_tiles, n_features]
    Sum over tiles axis -> [batch, n_features]
    """
    per_feature = {}
    for feat_idx in range(n_features):
        sp = {}
        for obs_key, tensors in relu_acts.items():
            vals = []
            for t in tensors:
...
pf_before = compute_relu_per_feature(acts_before['relu'], total_tiles, PRE_FTA_DIM)
save_heatmaps_pdf(pf_before, PRE_FTA_DIM, env, cfg,
...
pf_after = compute_relu_per_feature(acts_after['relu'], total_tiles, PRE_FTA_DIM)
save_heatmaps_pdf(pf_after, PRE_FTA_DIM, env, cfg,
