...
from representation_analysis import compute_sparsity_per_timestep, compute_sparsity_per_episode, compute_bin_counts_per_timestep
...
plot_dead_neurons_over_time(x=np.arange(len(sparsity_per_timestep_mean)), y =sparsity_per_timestep_mean, x_label='timesteps', y_label=r'% sparsity', se=sparsity_per_timestep_se, save=True, filename=os.path.join(FIGURES_DIR,args.model, f"env_{args.env_shape}", "sparsity_per_timestep.png"))
sparsity_per_episode_mean, sparsity_per_episode_se = compute_sparsity_per_episode(runs_out_arrays, thres=0.1)
plot_dead_neurons_over_time(x=np.arange(len(sparsity_per_episode_mean)), y =sparsity_per_episode_mean, x_label='episodes', y_label=r'% sparsity', se=sparsity_per_episode_se, save=True, filename=os.path.join(FIGURES_DIR,args.model, f"env_{args.env_shape}", "sparsity_per_episode.png"))
